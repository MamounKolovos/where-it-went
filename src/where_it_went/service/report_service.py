...
class ReportService:
    ...
    def process_chart_data(self, awards: List[Award], feature: str) -> Dict[str, Any]:
        if feature == 'award_amount':
            ...
            for range_item in ranges:
                count = sum(1 for award in awards 
                           if award.award_amount and range_item["min"] <= award.award_amount < range_item["max"])
                counts.append(count)
    ...
    def generate_summary(self, spending_response: SpendingResponse) -> Result[str, str]:
        try:
            summary = self.openai_service.generate_report(spending_response)
            return Ok(summary)
        except Exception as e:
            return Result.Err(f"Error generating summary: {str(e)}")